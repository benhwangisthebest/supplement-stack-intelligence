...
These are not conventions — they run on `npm test` via **seven** executable architecture specs **[2026-09-15, observed at U22: **21** · **22 at U32 (2026-09-18)** · **23 at U30 (2026-09-21)** · **24 at U33 (2026-09-21)**, the directory count; the figure here is the 2026-08-06 measurement and stays as written · **[2026-09-22, Phase 2 closeout] BOUND BY `SPEC_COUNT` at `30` — 24 → 26 when the two binding guards landed (they are themselves architecture specs), then → 27 at the (d1) remediation, which added `five-xx-is-logged.test.ts`, then → 28 at Phase 3 U1 landing (b), which added `canonical-layout.test.ts`, then → 29 at Phase 3 U9 landing (a), which added `client-props.test.ts`, then → 30 at Phase 3 U10 landing (c), which added `rule8-component-tests.test.ts`. Each step moved all four sites in the same commit, because the guard reddens otherwise — which is the first time this count has constrained a landing rather than recording one. Derived from `git ls-files` over `src/architecture/*.test.ts` and asserted against this site: a unit that adds a spec now reddens the build instead of appending a further value here. This is the last hand-written value; do not restack**]** under [`src/architecture/`](src/architecture) — **[2026-09-22, N-52 content half: the per-spec test counts that stood here are DROPPED rather than corrected. They were stale by 22 and 1, and a per-file count restacks a bracket on every unit that adds a test, which is the counts-written-once class at its finest grain. The directory count above is guard-bound; a per-file breakdown is not worth binding and is not worth carrying unbound]** — and are specified in [`docs/02-design/architecture-boundaries.md`](docs/02-design/architecture-boundaries.md):
