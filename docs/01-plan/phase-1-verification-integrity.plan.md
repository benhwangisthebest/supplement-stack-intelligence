...
No item is silently dropped. **[2026-08-06] Re-counted at closeout — the previous tally summed to 21 against a 19-row table.** **8** → Phase 1 unit (C-9, C-11, C-12, F6, D-2, D-4, the `boundaries.test.ts` claim→observed pass, and §4 rule 8 in part) · **5** → deferred with reason (F3, F5, F7, §4 rules 7 and 9) · **3** → ruled or cross-referenced (slug policy, U-DEFER-1, §4 rule 5) · **2** → closed/record-only (C-5, C-10) · **1** → excluded by user decision (C-6). **= 19 rows.** The buckets are disjoint; C-6 was previously counted twice.
...
| **C-9** `[LIVE]` tags | **CLOSED 2026-08-06 by U16** | Roadmap item 6 names it verbatim. Measured before: 17 gated files, **0** tags. Now **18 tagged blocks across 17 files** — the extra one because gating has TWO shapes, and the guard tags the gated BLOCK, not the file: 17 describe-level gates plus **1 test-level gate** in `medication-interactions.spec.ts`, whose parent describe (`L2: Library interactions (no auth)`) is genuinely ungated — two of its three tests run with no credentials. Tagging that parent to reach a file count of 17 would have stated a falsehood to silence a warning, which is the exact defect §5.9 exists to prevent. Enforced by `src/architecture/e2e-live-tagging.test.ts` (`LIVE_TAGGING`) in BOTH directions. |
...
Group E (cuttable)   U16 DONE 2026-08-06 → U17 PARTIAL 2026-08-06 [BLOCKED(env)]
   └ GATE E1  C-9 closed (all 17 gated files tagged) before any live baseline
              run, so the document can name what ran by tag.
              SATISFIED 2026-08-06 by U16: 17/17 files tagged (18 blocks), the
              tagging bound both ways by LIVE_TAGGING, and the shared-user race
              made structural (`workers: 1` under E2E_LIVE) so a live baseline
              cannot be recorded from a racing run.
```
...
- [x] `mappers.ts` ≥ 90 % statements; `execute.ts` ≥ 80 %; `validation/schemas.ts` ≥ 80 %. — **MET, re-measured 2026-08-06 at Phase 1 close: 100 % / 100 % / 100 % statements** (`vitest run --coverage`). Recorded honestly: this criterion was **satisfiable well before close and simply was never re-measured** after U7, U9 and U10 landed. Nothing was done at close to meet it. `execute.ts` sits at 87.87 % *branches*, which the criterion does not ask about.
...
- [x] Coverage thresholds configured for every pure engine directory and enforced by a CI step; no `branches` threshold within 10 pp of measured (D-2). — **MET 2026-08-05 by U13.** **14** pure engine directories carry thresholds in `vitest.config.ts`, every number `measured − 10` from a coverage run at the U15 tip. D-2 honoured twice over: the 10 pp margin is **500× the observed ±0.02 pp jitter**, and `src/lib/protocol-builder` carries **no `branches` key at all**, marked in-place so it is not "completed" later. CI gained a distinct `Coverage thresholds` step (`npm run test:coverage`); the four existing steps are byte-identical, +19/−0. **Six directories are deliberately excluded and named** (20 `src/lib/*` dirs − 14 thresholded): `src/lib/{auth,api,supabase}` (infrastructure, per the D-4 ruling), `src/lib/db` (persistence — 15.65 % statements, exercised through routes; FU-16), and `src/lib/{advisor,identity}` (each still holds a file importing `@/lib/db` — U18's subject). **[2026-08-06] This read "Five" while itself naming six**; the same off-by-one is in `5e5c943`'s commit message and in `vitest.config.ts`'s comment, so it propagated from the unit rather than originating here. **`package.json` needed no change**: the unit row lists it as modified, but `test:coverage` = `vitest run --coverage` already existed, and `@vitest/coverage-v8` is already a devDependency in the lockfile.
- [x] `[LIVE]` appears on every `E2E_LIVE`-gated describe (`grep -c '\[LIVE\]'` = 17, `grep -l E2E_LIVE | wc -l` = 17). — **MET 2026-08-06 by U16.** `grep -l '\[LIVE\]' tests/e2e/*.spec.ts | wc -l` = **17** and `grep -l E2E_LIVE tests/e2e/*.spec.ts | wc -l` = **17**, as written. **The criterion's block count is off by one, and the guard is right rather than the criterion:** total tagged blocks = **18**, because `medication-interactions.spec.ts` carries a *test-level* gate in addition to its gated describe. The criterion was written from a FILE count and silently assumed one gated block per file; the second gating shape was not enumerated when it was drafted. Recorded rather than reconciled by under-tagging (§6.2.2: a mismatch is a finding, not something to edit until it agrees). Now enforced both ways by `LIVE_TAGGING`: gated-but-untagged fails, and tagged-but-ungated fails.
- [x] Every guard added in this phase has its **red output recorded in `docs/`** — not self-reported in-session. This is the Phase 0 lesson (report §6) written as a criterion. — **MET 2026-08-06 by `docs/04-report/phase-1-verification-integrity.report.md` §3.** The previously-missing reds (U4, U7, U9, U12's marker, U19, U20, U21, U13's `HARNESS_GAP` and N1, U14's five, U16's six) are recorded there, and **every one was RE-EXECUTED against the certified tree on 2026-08-06 in a disposable worktree and pasted from the run**, not transcribed from a unit report — which is the distinction the criterion was written to enforce. Two discrepancies surfaced by re-execution are recorded rather than smoothed: U9's actual assertion reads `expected true to be false` where §6.2 predicted `expected success to be false`, and U14's M2 originally SURVIVED and was answered with a new rule (§6.2.2), not an edited test. U5/U6/U8/U10/U11/U15's reds were already in `docs/` at Gates A2, B1, C1/C2 and the criterion-5 annotation.
- [~] A dated live-E2E baseline exists, recording the exact commands, env-var **names** (never values), timestamps and per-spec results, with the superseded "61/71" and "79/10" figures struck through rather than deleted. — **PARTIAL 2026-08-06 by U17** → `docs/05-qa/phase-1-live-e2e-baseline.md`. **The NON-LIVE half is MET in full:** exact command, commit (`4246044`), UTC start/end timestamps, per-spec table (59 passed / 30 skipped / **0 failed**, 23 files), measured against a **production build** (U16's build-then-start, proven to have run via a deleted-`.next` cold start leaving `BUILD_ID`). Both superseded figures are struck through in `docs/roadmap.md` item 7, the only place they were being presented as current. **[2026-08-06 corrected at closeout]** This sentence previously read "and neither is cited again", which was **false**: `docs/reviews/mvp-transition-check.md:278` and `docs/01-plan/features/context-adjusted-evidence.plan.md:370` both still contain them. Both are dated historical records that §7 forbids rewriting, so they stay; the claim is narrowed to what is true — no document presents either figure as a current measurement. **The LIVE half is BLOCKED(env)** and no live figure was recorded or simulated: it needs a live Supabase project, `NEXT_PUBLIC_SUPABASE_URL`/`_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `API_ANTHROPIC_KEY`, migrations 0003–0007 and a seeded demo user — all owner-only. The document carries the exact commands to finish it. **The `fetch failed` artifact is investigated and CLOSED:** both failure modes reproduced from source without credentials — unset env gives `Supabase is not configured`, env-set-but-host-unreachable gives `fetch failed` exactly — so the 2026-07-30 artifact proves the env WAS configured and the host was unreachable. T-14 upheld, its severity understated. The artifact itself is gone and was never tracked (**FU-24**). **Reworded per ruling 5:** the roadmap's original "reproducible in CI" clause is **removed** — a CI E2E job is out of Phase 1's scope, so requiring it here would make the criterion unmeetable. Reproducibility in CI moves to whichever phase actually adds a CI E2E job.
...
- [x] `npx tsc --noEmit` clean, `npx vitest run` green, `npx next build` succeeds, CI green on the integration commit. — **MET 2026-08-06.** Group E integrated at `2443305`; `main` CI run `31033669440` **success**: 859 tests / 73 files, `✓ Compiled successfully`, every step green including `Coverage thresholds`. (`ci.yml` authors **7** steps, of which **5** are `run:` commands; the run page numbers **8** because it counts GitHub's own `Set up job`. Stated precisely because "8 steps" alone is ambiguous.)
...
| `advisor/conversations/[id]/route.ts` | `GET(_request, {params})` — request unused. **[2026-08-06] Updated after U21 landed (`882d53e`):** a foreign or unknown id now yields `notFound("Conversation")` — 404 — instead of the empty list RLS used to return. U21 added **no** 400 path, so this exemption still holds. *(This row read "today a foreign id yields an empty list" until closeout, which was written before U21 and never refreshed.)* |
...
| **FU-2** | U10 | `executeBatch`'s compensating rollback swallows rollback failures silently — no log, no correlation id — so a failed rollback leaves the stack half-applied with no trace. `respond.ts`'s `reportInternalError` exists for exactly this. | **ASSESSED by U11 2026-08-04 → now owned by U20.** U11 moved no part of `executeBatch`, so the risk is unchanged; what changed is that the fix got cheaper, since `src/services/advisor-actions.ts` already imports `@/lib/api/respond`. Recommended and scheduled as **U20** (log-only; the response contract is untouched). **[2026-08-08 → CLOSED.]** U20 shipped the log (`src/lib/advisor/actions/execute.ts:147`), re-verified at `d4f6194`. The re-label is carried by `docs/01-plan/phase-2-operational-dependability.plan.md` §4.3, which is **authoritative** for it; the certified sentence above is left exactly as certified. |
...
| **FU-22** | U13 | Deleting `src/lib/validation/schemas.test.ts` — **40 tests, all of U9** — did not trip the floor. **[2026-08-06 CORRECTED at closeout]** This row originally recorded the drop as "~0.01 pp" and as `schemas.ts` being "executed by 23 route test files". **Both figures were wrong.** Re-measured in a disposable worktree: `src/lib/validation` falls **97.96 % → 90.36 %** (−7.6 pp) and `schemas.ts` itself **100 % → 85.14 %**; **10** of the 23 route files import the module directly. The floor (87) still does not fire, so the row's conclusion stands — U9 added mostly *assertions*, not coverage. | **Deferred — a caution, not a bug, and now a second caution about this register.** The corrected numbers make the point *better*: a **7.6 pp** fall inside a directory can pass a floor set 10 pp below measured — precisely the D-2 trade-off FU-21 names. That the original figure survived from U13 to closeout unchallenged is itself the lesson: a follow-up row is a **claim**, and claims in this register were not being re-derived. Caught only because the closeout review re-ran it. |
| **FU-23** | U13 | U14's CI-step binding checks that the four declared steps EXIST in order; it does not check that no UNDECLARED step exists. U13 added a fifth (`Coverage thresholds`) and the guard stayed green — `CLAUDE.md` §5 was updated by hand, not because anything forced it. | **CLOSED 2026-08-06** by the U16 rider (`3108f55`). The binding is now an ordered EQUALITY between §5's declared chain and `ci.yml`'s `run:` steps, so an undeclared extra step fails exactly as a missing one does; a step named in §5 with no entry in `DECLARED_STEP_COMMANDS` is a hard failure too, so extending the doc cannot silently shrink the comparison. Four mutations red (extra, missing, unmapped, reordered) plus two anti-rot. 15 → 21 tests. |
| **FU-24** | U17 | **A review cited an on-disk artifact as evidence and nobody copied it into `docs/`.** `test-results/mvp-core-loop-e2e-…/error-context.md` (2026-07-30) is the artifact T-14 rests on. It is now gone: `test-results/` is untracked (only `.last-run.json` was ever committed, removed in `bf7ff2e`) and Playwright clears the directory at the start of every run. **U17's own verification runs may well be what destroyed it** — that cannot be ruled out and is not claimed otherwise. | **Open — process, not code.** The finding survived only because two documents quoted its text. Rule worth adopting: when a review cites an untracked artifact, copy it under `docs/` in the same commit. Cheap; would have preserved the primary evidence here. |
| **FU-25** | U16 | **Per-worker user isolation is still not done.** U16 killed the shared-user race by SERIALISING live runs, which is the available fix, not the complete one. Every authed spec still logs in as one seeded demo account, so a live run remains order-dependent and cannot be parallelised — and roadmap item 6 names per-worker fixtures as the prerequisite for a **CI E2E job**. | **Open — UNOWNED, blocks CI E2E.** Not attempted in U16: signing up a user per spec hits Supabase's email rate limits (the documented reason `helpers.ts` logs in rather than signing up), so real isolation needs seeded per-worker accounts — a change to the seed script and its service-role path, outside this unit's scope. |
| **FU-26** | U17 | **A fresh clone cannot run the E2E suite**: the pinned Playwright browser is not installed, and the failure presents as 36 specs failing at once, which reads like a catastrophic application regression rather than a missing binary. Observed here with a cache holding `chromium-1234` while `@playwright/test` wanted `-1223` — NEWER, not older, so "my browsers are installed" was misleading. | **Open — small.** A `postinstall` or a documented `npx playwright install chromium` step in the QA runbook would remove it. Not folded into U16/U17: adding a `postinstall` download to `npm ci` would change CI's install step, which needs its own behaviour-compatibility proof. |
| **FU-27** | closeout (c) | **The shipped nav renders four top-level items, not three.** `src/components/layout/TopNav.tsx` appends an `Advisor` pill to the same `NavPills` group for signed-in users. `CLAUDE.md` §1 and `docs/product-direction.md` both state three as permanent. The v6 design authorised a "top-level-adjacent surface … **not** a 4th main pillar", so this is an implementation divergence from the decision that authorised it, not unauthorised scope. | **Open — needs a product decision, deliberately not resolved at closeout.** The rule was **not** relaxed to match the code; that would rationalise the divergence (§8.1: name it, don't absorb it). Either move the Advisor out of the pillar group, or change the rule on purpose and update both documents. |
| **FU-28** | closeout (c) | **U19's "reported identically" claim is stronger than its guard.** The route answers `notFound("Stack")` vs `notFound("Item")`, and `notFound` writes `` `${what} not found.` `` into the client-facing `error.message`. The pin asserts equal **status** and equal **error code** only, so the messages differ and are unpinned. | **Open — small.** Residual disclosure is minor (learning "the item isn't in it" implies the stack *is* yours). Fix is one message for both plus a pin on it. Not taken at closeout: the closing commit is documentation-only, and changing a response body is a behaviour change needing its own unit. |
