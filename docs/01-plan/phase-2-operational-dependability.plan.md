...
|---|---|---|---|
| **N-1** | **`/api/lab-import/extract` calls a paid API with no budget check and no rate limit.** §4 rule 9 requires both. | `pdf-adapter.ts:136` imports `@anthropic-ai/sdk`; the route has zero budget references | → **U5, U7** |
...
**U5 · Rate limit both LLM-backed routes.** *(roadmap 3; N-1)*
...
**U6 · Disconnect, `maxDuration`, Anthropic timeout.** *(roadmap 3; named exit criterion)*
...
**U7 · `PAID_API_BUDGET` — the guard §4 rule 9 has never had.**
