...
**DONE 2026-08-12, `<COMMIT1>`.** Baseline before: typecheck clean, **1175/95**, non-live E2E 70/30.
...
**WHAT THIS UNIT DOES NOT CLOSE.** The exit criterion's second half — matching the **live** database —
is an owner-run sitting. `docs/05-qa/2026-08-12-deployed-schema-record.md` holds the procedure (read-only
statements, redaction posture, expected results, and a verdict table to fill in). **U15's closeout waits on
that record existing, the same way U27's waited on the activation smoke.**
