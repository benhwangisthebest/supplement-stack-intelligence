...
> ### The rows this register owed itself — FU-29, FU-30, FU-31, written 2026-09-22
>
> **[2026-09-22, Phase 2 closeout] LATE-REGISTERED, and dated as such rather than backdated.** N-3, N-4
> and N-7 each dispositioned *"Register as FU-nn"* and **no row was ever written**. Two of the three left
> no trace in the repository at all beyond the promise itself.
>
> **The mechanism, named because it is the register's own failure and not a clerical slip:** *"Register as
> FU-nn"* inside another row's disposition cell **reads like an action and is not one**. Four numbers were
> issued that way. FU-32 survived because U20 opened the file it named; FU-31 survived because a guard
> header recorded it. FU-29 and FU-30 had no such accident and did not survive. **Nothing distinguished the
> four at the moment they were written** — which is why the promise-shaped disposition is the defect, not
> the two rows that happened to fall through it. Recorded here so the next reader writes the row instead of
> the promise.
>
> - **FU-29** ← **N-3.** `mappers.ts` casts at **13** sites against plain `text`/`text[]` columns with no
...
>   unassigned; it is a deletion, not a design.
> - **FU-31** ← **N-7**, and unlike the two above it **is** recorded somewhere executable:
>   `src/architecture/error-disclosure.test.ts:92` — *"Recorded as **FU-31**; unclosed"*. Exactly **one**
>   module carries `"use server"` as its first statement (`src/lib/auth/actions.ts`), and it is an HTTP
>   endpoint that `AUTH_COVERAGE` (which scans `src/app/api/**/route.ts`) cannot see and that
>   `error-disclosure` does not reach on its non-caught reads. The ratchet pins the count at **1** so a
>   second cannot appear ungoverned. **Not a live leak** — the text is user-facing auth copy. **The blind
>   spot is the finding.**

> ### FU-35 … FU-38 — U31's four open C-table obligations, accepted as registered
>
> **[2026-09-22, Phase 2 closeout] Ruled by the owner: accepted with a written reason, NOT built.** Each
> carries the remedy U31 already proposed, so the next owner inherits a decision rather than a puzzle.
> **Owner for all four: the next operational phase.** None is a live defect; three of the four are
> instrumentation for work that phase will own.
>
> - **FU-35** ← U31 **C6.** **No guard covers `scripts/`.** **N-58 is what the absence already cost:** a
>   probe drifted one field away from production, 400'd, and printed a misleading diagnosis.
>   **Remedy, with the owner's constraint attached:** any guard proposed here must *at minimum* assert that
>   the probe scripts **import** their request bodies from `src/` rather than defining them.
> - **FU-36** ← U31 **C7.** **The `ClaudeAdapter` port name is two providers stale.** Re-derived at close:
>   `src/types/advisor.ts:124` still declares it, and `agent.ts`, `mock-adapter.ts` and `model-adapter.ts`
>   all name it. **Remedy: rename, or accept with the reason recorded** — the case for accepting is real,
>   since the rename opens `src/types/`, governed by §4 rules 1–2, for zero behavioural gain.
>   *(Consistent with §3's ports-and-adapters row staying **P**: the seam is P **because** it survived two
>   provider swaps; the stale type name is the residue of the second one, and is a name, not a seam.)*
> - **FU-37** ← U31 **C9.** **`.env.local` goes stale after a worktree split** (N-57). **Remedy, and the
>   constraint is what makes this row worth reading:** the obvious candidate — *loader warns on a populated
>   file with zero matching keys* — **would not have caught the observed case**, where three keys matched
>   and a fourth was swallowed. Whoever takes it must beat that counterexample, not restate the candidate.
> - **FU-38** ← U31 **C10.** **The lab-import probe verifies shape, never content** (N-61). **Remedy:** the
>   probe compares the transcript against the fixture's known synthetic values and prints only
>   match/mismatch counts — ***compare, never print***. That shape is what makes it buildable at all: it
>   closes the unmeasured dimension without putting health-shaped content into a record (§2.3 rule 15).

> ### FU-39 — raised on landing (a)'s own diff, and DROPPED here
>
> **[2026-09-22] Raised by `ecc:code-reviewer` reviewing landing (a); ruled DROP by the owner the same
> day, on FU-32's remedy.** Per-spec test counts stood at two sites outside `README.md` —
> `docs/project-status.md` and `docs/02-design/architecture-boundaries.md` — three of them stale
> (**22 · 8 · 2**) and four accurate. **Dropped rather than corrected**, because the total is bound by
> `SPEC_COUNT` and a per-file breakdown anywhere is the `README.md` defect one row down.
>
> **Why this row exists at all, when the fix is four deletions:** §10.3 recorded the two sites and said
> they would be registered for (b) — **and the row was never written**, which is **FU-29's exact mechanism
...
> ### FU-47 — one anchored comment-stripper, shared, instead of four private regexes
>
> **[2026-09-22] Registered at (d4) as N-79's shared remedy.** Four guards each carry their own
> comment-stripping regex; three use the non-anchored `` /\/\/[^\n]*/g `` and are blind to a `//` inside a
> string or URL literal (`boundaries.test.ts:1237`, `five-xx-is-logged.test.ts:194` and `:267-270`,
> `not-found-uniformity.test.ts:70`). **Seven other specs already use the anchored form**
> `` /^\s*\/\/.*$/gm `` and are immune: `nav-pillars`, `rendering-determinism`,
> `not-configured-totality`, `middleware-scope`, `ui-error-text`, `migration-tooling`, `route-contract`.
>
> **The remedy is convergence, not invention — the correct pattern is already in this repository, in the
> majority of its guards.** One exported helper, anchored, used by all eleven; or, where a guard needs to
> see code inside a line that also holds a string, a real tokeniser (the trade `readsIdentifier` made at
> (d1) after N-75, for the same reason).
>
> **Why it is a follow-up and not part of (d4):** changing what eleven guards read is a behavioural change
> to eleven controls at once, and the honest way to land it is one guard at a time with each one's set
> re-derived and diffed before and after. **(d4) disclosed the limit instead, which is the part that could
> not wait** — an undisclosed blind spot reads as coverage. **Owner: the next operational phase.**
...
| **N-11** | U2 | **`ExtractionError` now carries `cause` and nothing logs it.** U2 moved the underlying transcription error off the message and onto `cause` (removing the disclosure read). The diagnostic value is preserved *in the object* but never reaches a sink: the extract route answers a canned 502 and `handle()` is never reached, so `logInternalError`'s `describeCause` never runs on it | `pdf-adapter.ts:97,127` carry `cause: e` (the reads U2 removed were at `:92,119` before the fix); `extract/route.ts`'s `ExtractionError` branch returns without logging | **Open — blocked on a sink, which is roadmap item 1's residue.** Closes when **U23** lands (`path`, `userId`, a real sink). Recorded now because "the data is captured" and "the data is observable" are different claims and U2 only bought the first  **[2026-09-22, Phase 2 closeout] RE-OWNED — the closing condition became unreachable.** This row said *"Closes when **U23** lands"*; **U23 was cut** on 2026-09-14 (§9's cut order: U11, U21, U23), so since that date the row has been open against a unit that will never exist. **New owner: the phase that adds a logging sink**, carried as a residue **together with U23's own** (§10.7) rather than separately, because they are one piece of work seen from two directions. **Not closed, because the finding is still true:** `ExtractionError` carries `cause` and nothing logs it. U2 bought *the data is captured*; nobody has yet bought *the data is observable*. **The general lesson, recorded because this is not the only row that could have it:** a disposition that names a *unit* rather than a *condition* inherits that unit's mortality. |
...
| **N-75** | **Landing (d1)'s own red proof for P2-R3, 2026-09-22** — the planted violation did not redden | **`boundaries.test.ts`'s `readsIdentifier` stopped scanning at the first template literal with a substitution, blinding FOUR credential ratchets.** It used `ts.createScanner` with a bare `scan()` loop, which carries no template-continuation state: on reaching `` `…${…}…` `` the scanner needs `reScanTemplateToken`, never gets it, and the walk terminates. Everything after that point in the file was unread. The four pins over `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENAI_MODEL` and `SUPABASE_SERVICE_ROLE_KEY` were therefore green over files they had not finished reading | Minimal repro at (d1): a fixture whose identifier sits after a substituting template literal returns `false` on the old helper and `true` on the new. P2-R3's planted read in `src/lib/db/seed.ts` failed to redden the ratchet, which is how it was found — not by review | **CLOSED BY (d1), `bac5928`.** Rewritten as an AST walk (`ts.createSourceFile` + `ts.forEachChild`), matching identifiers and string/no-substitution-template literals, with ~~**two regression fixtures**~~ **one regression fixture** pinning the exact shape that blinded it. **[2026-09-22, (d4), on N-78] ~~two regression fixtures~~ ONE.** The second — *a read INSIDE a template substitution* — was tested against the reconstructed old scanner at (d3) and was **already green against the bug**, so it never pinned N-75. It is retained and retitled as a coverage case. **(d1)'s commit message says "two regression fixtures" and cannot be changed**; this row is the correction, which is what a register is for. **Certified sound at (c) on 64/64 green, which is the finding's real content:** the Check's security section read these four ratchets and could not have seen this — a blinded guard and a satisfied guard are the same colour. **The (d3) addendum must state that and re-verify all four on the rewritten helper** |
...
| **N-78** | **`ecc:security-reviewer` at (d3)**, re-verifying N-75's four ratchets on the rewritten helper; spot-verified against source by the certifier | **Of the TWO regression fixtures N-75 claims, only ONE pins the bug.** ~~The (d1) commit message, N-75's register row and §10.11 all say *two regression fixtures*.~~ **[2026-09-22, (d5)] CITATION CORRECTED — the third source does not exist.** The certifier's closing pass (§7.7) re-derived it: **§10.11 has never contained fixture-count language**, at `ee9db07` or before. The two real sources are **(d1)'s commit message** (*"with two regression fixtures"*) and **N-75's register row**, both now dated to one. *The row that exists to correct an overclaim carried an overclaim of its own — a citation asserted rather than re-derived, which is the same act in miniature. Struck rather than rewritten (§7), because the correction is the record.* Reconstructing the pre-`bac5928` scanner verbatim and running both against it: *"a read AFTER a template literal with a substitution"* returns `false` — **red against the old code, a genuine guard**; *"a read INSIDE a template substitution"* returns `true` — **green against the bug it is filed under**, because the old tokeniser reads `process.env.OPENAI_API_KEY` as ordinary identifiers before reaching the `CloseBraceToken` that triggers the mis-tokenisation | Old helper reconstructed from `git show bac5928^:src/architecture/boundaries.test.ts` into a disposable scratch script; fixture A `false` (expects `true`) → would have reddened; fixture B `true` (expects `true`) → would not. Fixtures at `boundaries.test.ts:1611-1616` | **CLOSED BY (d4) — RELABELLED, NOT DELETED.** Fixture B is a real correctness check for an adjacent property (a read *inside* a substitution expression) and is kept **as a coverage case**, retitled so it no longer reads as an N-75 regression guard. N-75's row and §10.11 are dated to **one** regression fixture. **The (d1) commit message cannot be changed and carries the overclaim** — noted here because an immutable record that overstates is exactly the thing a register exists to correct without rewriting. **The class is §5 rule 2 one layer up:** a *fixture* written to pin a bug was itself never shown red against that bug, which is the same omission the rule names for guards |
| **N-79** | **`ecc:security-reviewer` at (d3)**, sweeping for N-75's class after discharging it | **A non-anchored `//`-strip blanks the rest of any line containing a URL literal, and it is live and UNDISCLOSED in three guards.** `.replace(/\/\/[^\n]*/g, " ")` cannot tell a line comment from `//` inside a string, so a line such as `docs: "https://example.com/models", fallback: "claude-haiku-4-5-20251001"` is blanked from `https:` onward and the real literal beside it is never scanned. **Not the N-75 mechanism** — nothing terminates early — but the same family: *match prose, not code* | Empirically confirmed blind at `boundaries.test.ts:1237`'s `stripComments`, which backs `NO_PINNED_MODEL_ID`'s *"no model id hardcoded anywhere in `src/`"* offender scan — **and that guard's own "HONEST LIMITS" comment names other limits and not this one.** Also undisclosed at `five-xx-is-logged.test.ts:194` and `not-found-uniformity.test.ts:70`. **Seven specs are immune**, using the anchored `/^\s*\/\/.*$/gm`: `nav-pillars`, `rendering-determinism`, `not-configured-totality`, `middleware-scope`, `ui-error-text`, `migration-tooling`, `route-contract` | **DISCLOSED AT (d4); the shared fix is FU-47.** Each of the three carries the limitation in its own header or HONEST LIMITS comment now, because **a guard's blind spot belongs beside the guard, not in a review nobody opens again**. No live undetected violation exists in tracked source today — checked. **What makes this worth a row rather than a comment:** `five-xx-is-logged.test.ts` held **both** an undisclosed instance (`:194`, pre-existing) **and** a disclosed one (`:267-270`, added at (d1b) on a reviewer's LOW) — the same blind spot, in the same file, one written down and one walked past, by the landing that wrote the disclosure |
...
| **OP-6** ✅ **DISCHARGED 2026-08-10** — `docs/05-qa/2026-08-10-rate-limit-policy-verification.md`, hours after this row was opened. All four as `authenticated`: DELETE (**no `WHERE`** — every visible row targeted) removed 0; INSERT raised **`ERROR 42501 new row violates row-level security policy for table "api_rate_limits"`**; the own-row SELECT returned rows; and `consume_rate_limit` granted and then refused at the cap. **[2026-09-22, Phase 2 closeout] MOVED INTO NUMERIC ORDER.** This row printed *after* OP-7, so a reader scanning for the last row found the second-to-last. An append-order artifact of a row edited after it was written — the same class as U31's C-table, which prints `C1…C7, C10, C9, C8` and is noted there. Neither ever changed a disposition; both cost a reader a double-take |
...
| **N-22** | ~~**OPEN.** `auto/*` aliases complete a tool loop and return an **empty** answer — two aliases, two vendors.~~ **[2026-09-22, Phase 2 closeout] STALE — THIS SUMMARY CONTRADICTS THE REGISTER, and §4.5 is authoritative.** The claim was **re-scoped and withdrawn on 2026-08-10**, the same day it was written: the owner ran `auto/best-free` end to end in the deployed UI and it answered, and the cause was found in the probe (step 3 feeds a fabricated tool result under a system prompt that forbids guessing, so an empty second step is a plausible *correct* response). **What survives is in §4.5, not here.** Dated rather than rewritten, per §7 — and left in place because a unit summary that outlived the finding it summarises is worth a reader seeing once |
...
*(**[2026-09-21] U34 DID THE CONVERSION THIS PARAGRAPH DECLINED, and the paragraph stays as written.**
It was right on its own terms — U30 was a twelve-site validation change and had no business rewriting
one handler's error contract, which is why N-72 was registered instead of absorbed. U34, which owned
that contract, found the reason U30 could not have known: `await params` and `await createClient()` ran
**outside the try**, so a throw at either escaped into a non-envelope 500 with no correlation id. The
route is now on `handle()` with `{ code: "UNDO_ERROR" }`, and `uuidParam.parse(id)` replaced the
`safeParse` special case this paragraph proposed. **U34's M9 pins that the 400 is byte-identical either
way**, which is what makes the special case's removal safe rather than merely tidy — and is the
retrospective proof that this paragraph's "identical 400 body" claim was true.)*
...
3. **N-72 is untouched** — the undo route still does not use `handle()`; that is U34's. *(**[2026-09-21]**
   And U34 did it: the route is on `handle()` with `{ code: "UNDO_ERROR" }`. This line stays as written
   — it was true of the tree U33 left — with the pointer added so a reader does not carry it forward.)*
...
#### **[2026-09-21] U34 — UNIT REPORT. DONE.** A failed batch reports the state it left

**STAMP ROW.**

| | |
|---|---|
| **commit** | `f060620` — 10 files, **+1026 / −57** |
| **branch run** | **`35666152700`**, green, **18/18 steps** |
| **post-merge run** | **`35666399605`**, green, **18/18 steps**, on `f060620` |
| **merge** | fast-forward `f01c69a` → `f060620`; branch `feat/u34-honest-failure-state` deleted; `main` the only ref |
| **CI figures, read through `gh`, each checked against a local run** | lint **366 of 366 tracked files, 0 errors** · vitest **1406 passed / 111 files** · non-live E2E **70 passed / 30 skipped**. **All three match local exactly**, and the E2E figure was re-run locally for THIS unit rather than carried over from U33 — the same rule that row set |
| **suite** | 1386 / 111 → **1406 / 111** |
| **specs** | `src/architecture/` unchanged at **24** — U34 adds no spec, so the closeout sweep has nothing to move on that line |
| **bkit** | `u34-honest-post-commit-failures` → `completed` |

**N-71 → MITIGATED at the API (FU-34 owns the product half). N-72 → CLOSED. N-74 → CLOSED.** All three
struck with dated dispositions in §4.5 in this unit's own commit.

**WHAT THE UNIT IS ACTUALLY ABOUT, in one sentence:** for `remove_item` and `edit_item` the prior dose,
unit, timing, reason and notes exist **only** inside the unpersisted inverse, so a `recordBatch` failure
was an irreversible destructive write to health data with no record that it happened — and the response
said the same thing it says when nothing was written at all.

**THE THREE LESSONS, and none of them is the feature.**

**1. A REMEDY APPLIED TO THE HALF THAT WAS REPORTED (N-74).** T-06, 2026-07-30, found `executeBatch`
swallowing rollback failures with no trace and recommended one thing: **log it**. U20 logged it. Nobody
went back to the response, which kept returning `rolledBack: true` unconditionally — so for thirteen
months the fix sat beside a claim it contradicted, and the comment above that line called it *"a
computed fact the client acts on"* when it was neither computed nor acted on. **A correction that names
a remedy gets that remedy**; the half nobody wrote down stays broken, with a green suite over it.
`mvp-transition-check.md` is annotated rather than rewritten, because the row was right and the gap is
in what happened next.

**2. AN ASSERTION ABOUT WHAT A HANDLER SAYS IS NOT AN ASSERTION ABOUT WHAT IT DOES (M2).** Making the
audit-path rollback a no-op, while still answering `rolledBack: true`, left the suite **fully green**.
The test asserted the response shape and never that the revert ran — so it would have passed against a
handler that claims an undo and performs none, which is **this unit's own defect, inside this unit's own
red**. Fixed with a reachability assertion (§5.3). **Third appearance this phase:** U29 needed it for
ordering, U33 for which condition failed, U34 for whether the work happened at all.

**3. "UNREACHABLE" IS A PROPERTY OF TODAY'S CALL SITES, NOT OF THE FUNCTION.** Both reviewers
independently found `withRollbackOutcome`'s `new Error(String(err))`, and `logInternalError`'s non-Error
branch — in the file I was writing against — says *"the value itself is never copied, because nothing
here knows what it holds."* My line lifted that value into a `message` the logger would then write in
full; an object with a custom `toString` is all it would take. The code reviewer advised a comment,
because no throw site rejects with a non-Error **today**. That is true, and it is the same argument that
made the line easy to write. **Fixed, not annotated.**

**THE RULING THAT WENT AGAINST THE OWNER, AND WHY IT IS RECORDED AS A DELIVERY ARGUMENT.** The proposal
— return the unreverted inverse payloads to the authenticated owner, their own data, the last place it
can survive — was refused on `ecc:architect`'s objection. **Not on privacy:** every read is owner-bound
and RLS-backed, and the owner's reading of §2.3 rule 15 (it governs the log, not the owner's own
response) is sound. **On delivery:** nothing in `src/components` reads `error.details` — the only
consumer of this endpoint's error body is `ActionProposalCard.tsx:76-78`, reading `error.message` — and
there is no client-side error reporting to capture it. The payloads would be serialised, delivered and
dropped. **That is this repository's own test for a guardrail — one that does not run does not exist
(§10.3) — applied to a recovery affordance.** FU-34 carries the condition under which the instinct wins.
**The owner's log test was built anyway and is the stronger half**, with M10 proving it can see a leak.

**THE SWEEP — four greps, four sites that were true when written and are false now. Every one annotated,
none rewritten (§7).**

| site | what it said | treatment |
|---|---|---|
| `mvp-transition-check.md` T-06 | *"Recommended correction: log the rollback failure"* | **annotated** — a certified review artifact (D-3). The recommendation was right; the annotation records that it was executed in two instalments 13 months apart and corrects its second half: **`replaceFlags`' atomicity was DELIVERED by U8** — insert-then-delete-by-id, with the §8 criterion shown red against delete-first — and what survives is the **transactional** residue U8 itself framed and accepted: duplicates on a failed delete, which is recoverable where loss was not |
| plan, U30 PLAN block `:3377` | *"no conversion of that handler to `handle()`"* | **annotated** — U30 was right to decline: it was a twelve-site validation change with no business rewriting one handler's error contract, which is why N-72 was registered instead of absorbed |
| plan, U33 non-coverage | *"the undo route still does not use `handle()`; that is U34's"* | **annotated** with the pointer, so a reader does not carry it forward |
| `u30-uuid-path-params.plan.md` RISK row | *"the twelfth handler is not like the other eleven"* | **annotated** — it was the risk U30 faced |

**`rolledBack` prose outside the plan: none.** **`UNDO_ERROR`: every live site is correct** — the code
survived the move, which was the point of adding `{ code }` to `handle()` rather than accepting the
re-label.
...
- [x] **Zero `for all` policies on constraining-counter tables.** Check: ~~**`! grep -q "for all"`** over
      ...
      **inverted**: the better a migration documents its own reasoning, the more certainly the check
      ...
      ✅ **MET — ticked 2026-09-22 at landing (b).** `RLS_COVERAGE` **22/22** green with a non-empty policy inventory; `advisor_usage` is `for select` only after `0008`, `api_rate_limits` (`0009:59-60`) likewise, each with a `security definer` writer. Red: **M8**, both `git add -N` ways. Evidence: §10.2.
- [x] **Every `SECURITY DEFINER` function sets `search_path`.** Check: `SQL_FUNCTION_REGISTRY` green, and
      its inventory is non-empty (anti-vacuity).
      ✅ **MET — ticked 2026-09-22 at landing (b).** `SQL_FUNCTION_REGISTRY` **24/24** green, including the anti-vacuity `it("finds the functions it is supposed to govern")` and the assertion that no definer function takes the caller's identity as a parameter. Evidence: §10.2.
- [x] **[P2-X2]** **A concurrency test proves the daily token budget cannot be exceeded**, shown **red** against the
      ...
      ✅ **MET — TICKED 2026-09-22 AT LANDING (b), which is the landing that created the instrument this
      criterion names.** The test is green and its fake is **stateful and yields at the start of every
      operation** — without that yield the old implementation passes too and the test proves nothing, which
      is Phase 1 U10's §6.2.2 lesson applied rather than quoted. **Both races are red-evidenced**, and the
      red text is now in the phase report as the criterion requires:
      `docs/04-report/phase-2-operational-dependability.report.md` §3.1 — race 1
      (`getRemainingBudget` → `recordUsage`) at **M14**, `expected [400,400,400,400,400] to have a length
      of 2 but got 5`; race 2 (select-then-upsert **inside** `recordUsage`, which *lost* usage) at
      **M15**, `expected 300 to be 600`. *(This criterion could not close before the report existed. That
      is not an oversight in the sequencing — it is the sequencing: a criterion that names an artifact
      cannot tick until the artifact does.)*
- [x] **[P2-X3]** **Both paid-API routes enforce a rate limit and a budget reservation**, where "paid-API route" is
...
- [x] **No paid call bypasses the one client module, and `@anthropic-ai/sdk` is gone.**
      ...
      **The `package.json` clause is kept exactly as written** — it is the half the literal form gets
      ...
      ✅ **MET — ticked 2026-09-22 at landing (b).** `SOLE_PAID_CLIENT` ×3 and `RETIRED_PACKAGE` ×2 green; `package.json` clean in both dependency blocks. Red: **M5** (inline `fetch`), **M7** (the address half). Evidence: §10.2, and the struck literal check above.
- [x] **A turn whose provider response omits `usage` settles nothing.**
...
- [x] **[P2-X4]** **Client disconnect terminates the advisor loop and settles its reservation.** Check: two assertions
      ...
      ✅ **MET — ticked 2026-09-22 at landing (b).** `src/app/api/advisor/route.test.ts:411-463`, two assertions with `status: "aborted"` — the adapter call count and `settleUsage` called with the reserved amount, which is the pair this criterion names. Evidence: §10.2.
- [x] **[P2-X5]** **`replaceFlags` leaves prior flags intact under induced insert failure.** Check: the test exists and
      was shown red against delete-first.
      ✅ **MET — ticked 2026-09-22 at landing (b).** `src/lib/db/evaluation-flag-repo.test.ts:222-247`, shown red against delete-first. *(The transactional residue is carried openly in §10.7: the operation is still three round trips with no transaction around them. **U8 met the criterion as written**, and the criterion is narrower than "atomic".)*
- [x] **[P2-X1]** **`error-disclosure` scans `src/app/api/**`, `src/services/**` and `src/lib/**`**, with a violation
      ...
      **one** `"use server"` module of N-7 remain unscanned.)*
      ✅ **MET — ticked 2026-09-22 at landing (b).** Binding-count grep = **3** in the corrected, non-vacuous form; spec **31/31** green; violation list `[]` and **the allowlist was never created**. Red: **M6**. *(Stated non-coverage unchanged: `src/app/**/page.tsx`, `src/components/**`, and FU-31's one `"use server"` module.)*
...
- [x] **`npx tsc --noEmit` clean · `npx vitest run` green · `npx next build` succeeds · CI green on the
      integration commit**, with the suite count recorded at close and re-measured, not copied.
      ✅ **TICKED 2026-09-22 AT LANDING (b), CONDITIONALLY — "on the pushed SHA's run", and (c) confirms
      it.** The condition is stated rather than assumed because **this criterion's own SHA is the one it
      measures**: the integration commit is **(b)'s**, and (b) cannot observe its own CI run before it
      exists. Locally, on the tree (b) produces: `tsc` clean · `vitest` green · `next build` succeeds.
      **The figures are re-measured and not copied** — landing (a)'s were 1420/113, and (b) adds no test,
      so the report's §1 header records (b)'s own measurement beside (a)'s rather than inheriting it.
      **(c) verifies this row against (b)'s pushed SHA and its run**, and a red run there reopens this tick
      rather than being absorbed. *(Ticking on a condition is a deliberate choice over leaving it unticked:
      an unticked criterion whose work is complete is what left C15 and `[P2-X6]` invisible for months.
      The condition is written into the tick so it cannot be read as unqualified.)*
- [x] **Every guard added in this phase has its red output recorded in `docs/`** — the Phase 1 criterion,
      retained because it is the one that produced the most value.
      ✅ **MET — TICKED 2026-09-22 AT LANDING (b).** **359** `M<n>` mutation references across this plan,
      **compiled** into `docs/04-report/phase-2-operational-dependability.report.md` §3 — §3.1 route and
      engine units, §3.2 architecture guards, §3.3 **surviving mutations, recorded and not fixed**.
      *(§3 is compiled from the per-unit red tables and the `docs/05-qa/` records and **cites** them; the
      mutations are **not re-run** at close. Stated so the report's §3 is read as an index of evidence
      produced at execution time, which is when a mutation is meaningful, rather than as a fresh run.)*
      The two guards added by landing (a) carry their own red tables at §10.10 — **M1–M4**, reverted by
      file-copy backup.
...
- [x] **The follow-up register is complete and each row re-derived at close.** Check: FU numbering
      ...
      **Owner: the closeout. Neither is built now.** *(Why here rather than as two new register rows: both
      ...
      ✅ **MET — TICKED 2026-09-22 AT LANDING (b), and it is the criterion this closeout most nearly
      failed.** **Contiguity, re-derived not re-typed:** N **1–74**, OP **1–7**, FU **1–39**, no gaps and
      no duplicates. *(Caveat for the next guard author, recorded because a naive check reports it as a
      defect: `grep -oE '^\| \*\*N-[0-9]+\*\*'` reports N-21…N-24 **twice** — the second occurrence of
      each is U25's **two-column** closeout summary, not a register row. Key on the register tables'
      five-column shape or on section bounds.)*
      **The clause was FALSE when this closeout opened**: FU-29, FU-30 and FU-31 had been promised in
      writing and never written, and the register that was supposed to catch that is the register that
      lost them. All three are written at this landing, **dated as late-registered rather than backdated**,
      so the gap stays visible in the register that failed to prevent it. **Every row re-derived**: eight
      stale N rows re-dispositioned against their landing commits, N-11 re-owned after its closing unit was
      cut, N-50 deferred to Phase 4, N-52 closed in both halves, FU-35…FU-38 accepted as registered, and
      **FU-39 raised on landing (a)'s own diff by `ecc:code-reviewer`** — the promise-shaped disposition
      reproducing itself inside the commit that fixed it.
      **Both binding guards this criterion names are built and red-proven**: `CRITERIA_PARITY` and
      `SPEC_COUNT`, landing (a), `749dbfc` — see §10.5 and §10.10.
...
| **`replaceFlags` transactional residue** | U8 **met the criterion** (prior flags survive an induced insert failure); the operation is still three round trips with no transaction around them. **[2026-09-22, (d4)] Owner-condition:** *the next unit that adds a second writer to `evaluation_flags`, or any change that makes a partial write observable to a user* — today the three round trips fail safe because nothing reads between them |
...
| **U-DEFER-4** | Outstanding by dated exception since Phase 1; `HARNESS_GAP` still hard-fails a tracked `*.test.tsx`. **[2026-09-22, (d4)] Owner-condition, written here because a dated exception is not an owner:** *the phase that introduces component testing*, which `docs/roadmap.md` places in **Phase 4** — **so Phase 3 inherits a `test-verified` UI criterion whose verification method a later phase owns** (P2-6). Either Phase 3 opens by building the harness or that criterion is re-sequenced; the choice is the owner's |
...
| **FU-29**, **FU-30**, **FU-31**, **FU-35**, **FU-36**, **FU-37**, **FU-38** | Registered at (b); owners as recorded in §10.3. **[2026-09-22, (d4)] The range form `FU-35 … FU-38` that stood here is expanded to its members** — a range is not a row, and a parse over this table returned FU-35 and FU-38 and silently dropped FU-36 and FU-37. That is **FU-46**, occurring in the table that is supposed to be the definitive list |
| **FU-40** — `RLS_COVERAGE` sees policy existence, not policy semantics | **[2026-09-22, added at (d4)]** **Owner: the phase that changes how migrations are verified.** CI's `pg_policies.cmd` catalog check is the control; the condition that would reopen this is CI ceasing to apply migrations |
| **FU-41** — health-bearing error text can reach a log record (**N-40**'s class) | **[2026-09-22, added at (d4)]** **Owner: the phase that adds a logging sink** — the first point at which a redaction layer has somewhere to live |
| **FU-42** — `CRITERIA_PARITY` cannot express a partial criterion, so it shapes the record it measures | **[2026-09-22, added at (d4)]** **Owner: the next unit that touches `criteria-parity.test.ts`**, or Phase 3's first closeout |
| **FU-43** — `src/middleware.ts`'s `getUser()` has no guard and no window to put one in | **[2026-09-22, added at (d4)]** **Owner: the phase that adds a logging sink** — the missing half is the sink, not a `try/catch` |
| **FU-44** — the correlation-id contract ends at `handle()`'s reach (three surfaces) | **[2026-09-22, added at (d4)]** **Owner: the phase that adds a logging sink**, beside N-11 and FU-43 |

> **[2026-09-22, (d4)] WHY SIX ROWS WERE ADDED HERE AND NOT AT (d2).** FU-40…FU-44 were registered at
> (d2) in §4.3 and never added to this table; FU-45 and FU-46 were registered at (d2) and never added
> either. **That is P2-2's exact pattern — a row that exists in the register and not in the list of what
> survives — recurring in the landing that fixed P2-2, and again in the landing that registered the rows.**
> Found by the certifier at (d3) and by rebuilding this list **from this table by command** at (d4)
> instead of from the previous list. **FU-39 is deliberately NOT here: it was ruled DROP the day it was
> raised, and a dropped row does not survive a phase.**
...
| **FU-45** — cycle artifacts have a stated cap and nothing measures it | **[2026-09-22, added at (d4) on the certifier(d3) finding]** **Owner: the next operational phase.** Remedy: a length assertion on `docs/01-plan/features/*.plan.md` built on `SPEC_COUNT`'s pattern, with the globbed set pinned non-empty |
| **FU-46** — the register uses four row shapes; a section-bounded parse gave three wrong answers before it was sound | **[2026-09-22, added at (d4)]** **Owner: the next operational phase.** Remedy: one declared row shape and a guard keyed on it |
| **FU-47** — four private comment-strippers, three of them non-anchored and blind to a `//` inside a URL literal (**N-79**) | **[2026-09-22, added at (d4)]** **Owner: the next operational phase.** Remedy is convergence, not invention: seven sibling specs already use the anchored form |
...
**bkit:** registered as `phase2-closeout`; artifact `docs/01-plan/features/phase2-closeout.plan.md`,
subordinate to this entry (`CLAUDE.md` §9).

| bkit | value |
|---|---|
| **feature** | `phase2-closeout` |
| **phase** | ~~`plan`~~ → **`completed`**, advanced 2026-09-22 at (d5), **by the session that closed the cycle** — §9's condition, not a later tidy-up |
| **where the state lives** | `.bkit/state/pdca-status.json`, **gitignored** (`.gitignore:74`), so it is local to this machine. **This row is the tracked record**; the JSON is not |
| **why the row exists at all** | §9's retired note returns as written the moment a unit ships without registering. A tracker current for some units and silent for others is the half-use the note forbade |
...
| **(d3)** | `26814bd` | branch **35795630536** ✓ · post-merge **35795961371** ✓ | 12 / 18 · **GATE D1: `ci.yml` 0 lines** | lint **369 of 369, 0 errors** · vitest **1446 / 114** · rendering determinism **no prerendered page HTML** — all three identical to local |
| **(d4)** | `ee9db07` | branch **35798806262** ✓ · post-merge **35799106411** ✓ | 12 / 18 · **GATE D1: `ci.yml` 0 lines** | lint **369 of 369, 0 errors** · vitest **1446 / 114** · rendering determinism **no prerendered page HTML** — all three identical to local. **CI-only, stated:** E2E non-live **70 / 30 skipped**, not re-run locally because `src/app` is untouched |
| **(d5)** | *this landing* | recorded on push; the phase's last | 12 / 18 · **GATE D1: `ci.yml` 0 lines** | local: tsc clean · lint **369 of 369, 0 errors** · vitest **1446 / 114** · build exit 0 |
...
#### Postscript — `CRITERIA_PARITY` caught a live divergence at landing (b), five hours after it landed

**Not a mutation. Not a drill.** While ticking criteria at (b), this closeout ticked `[P2-X2]` (the
concurrency criterion) in §8 and **not** its roadmap twin. The guard refused:

```
AssertionError: tick state diverges — P2-X2: §8=[x] roadmap=[ ]:
  expected [ 'P2-X2: §8=[x] roadmap=[ ]' ] to deeply equal []
```

**This is the exact defect the guard was built for, committed by the session that built it, within hours
of building it, while holding the whole argument for it in mind.** The two ticks were one edit apart. The
roadmap copy is now ticked in the same landing, with U4's red evidence beside it.

**What it settles, and it is worth more than the four planted mutations above:** the case for `[P2-X7]`
and `[P2-X8]` drifting was always that nobody was *reading* both lists. This shows the weaker and more
uncomfortable version — that **reading both lists is not enough**, because someone who has just spent a
day on precisely this failure still makes it on the next page. §3 rule 5's *"prefer a test over a
paragraph"* is usually argued from the case where the paragraph is unread. The stronger argument is here:
the paragraph was written, understood, and still not obeyed.

**[2026-09-22, SECOND CATCH, same landing.]** It then did it **again**, four ids at once. Ticking the nine
measured-MET criteria in §8 left their roadmap twins unticked — `[P2-X1]`, `[P2-X3]`, `[P2-X4]`, `[P2-X5]` —
and the guard named all four in one message. **Two live catches in one landing, by the session that built
the guard, with the argument for it still on screen.**

**And the second catch found something the first could not: `[P2-X1]` was a MIS-PAIRING, not a drift.**
§10.4 had already recorded that the roadmap's R1 carries a clause C10 does not — *"every 5xx has a
correlating server-side log entry with a request ID"* — and filed it as *"text divergence, not tick
divergence."* **That phrase quietly assumed the two would never need to tick together.** They did, and the
guard forced the question the note had deferred: **is R1 met?**

It is, **as written**: `handle()` writes a structured record carrying a correlation id on every unexpected
error. Item 1's *"target a real sink, not `console.error` alone"* is undelivered, and that is **item 1's
residue, not the criterion's** — carried as N-11 + U23's cut, and the reason Observability is classified a
**bounded refactor** rather than production-suitable. Both are now written beside the tick in
`docs/roadmap.md`.

**A tick-state guard surfaced a specification question.** That is more than it was built to do, and it is
