...
| **FU-48** *(new, landing (c))* | **OPEN — owner U2, and NO LONGER CONDITIONAL.** D-2 put the corpus in a root `content/` package, which is exactly the branch that takes it outside G1's `src/`-only, `.ts|.tsx`-only walk. U2 re-points both and proves it red. See §3. |
...
| **FU-49** *(new, U1 landing (a))* | **OPEN — owner U2.** Where editorial notes live in the JSON format. One product comment (glycine, *"target 3 g"*) is not held by seed data and is recorded verbatim in the U1 cycle artifact. See §3. |
