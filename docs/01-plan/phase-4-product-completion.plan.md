...
|---|---|---|---|---|---|---|---|
| **U0** Local gate gains `test:coverage` · **SUPERVISED** | **D-15 (b), owner clarification:** add `npm run test:coverage` to G, propose the `CLAUDE.md` §5 rule 10 diff, and **stop for the owner's approval of that diff** before U1 runs | det | S | `CLAUDE.md` (§5 rule 10, on approval only) | D-15 | none (no guard); `npm run test:coverage` green at HEAD | **yes** (the `CLAUDE.md` diff) |
...
| **U7** Sourcing pass · **OUT (D-3)**: FU-62's sourcing → roadmap backlog | Glycine-sleep and zinc-deficiency by Phase 3 U6's process: verified abstracts, captured responses, no model recall | **live** (network) | M | `content/seed/seed-papers.json`, `content/verification/**`, `docs/05-qa/**` | FU-62 (sourcing) | P3/P7/P8 stay red on a planted unverified entry at the new ids | **yes** |
...
| **U11** *(D-4 (a) + (c)(ii))* API voice · **SUPERVISED** | The 15 route sites keep their per-resource wording, which is recorded as the voice. **The three service 404s stop echoing the supplement id** | det | S–M | the 15 route files, `src/services/advisor-actions.ts`, their tests | N-50 | Under every option, `NOT_FOUND_UNIFORMITY` stays red on a response that distinguishes another user's resource from a missing one | **yes** (copy) |
...
| **U16** *(D-8)* DB-value integrity · **RUNNER** | **FU-29 (a) mapper validation (+ FU-17) only.** FU-57's row is kept. **No migrations** | det (a) · **live, deployed DB** (b, FU-57) | M (a) · L (live) | `src/lib/db/mappers.ts`; `supabase/migrations/**` under (b) | FU-29, FU-17 | (a): a mapper test red on a planted out-of-domain row value. (b): migration coherence (CI Postgres, `verify:migrations`) red on the same value | no; OP row + dated record for live paths |
