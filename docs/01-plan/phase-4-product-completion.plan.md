...
| Item | What | Position |
|---|---|---|
| **N-85** + criteria parity *(unnumbered)* | DOC_TRUTH blind to `describe`-title guards · `[P3-Xn]` absent from the roadmap | **N-85 IN U1.** **Parity IN U1 only under D-11 (a)/(b)**; under D-11 (c) it stays absent (P-16). **CLOSED by U1 (2026-09-25):** N-85 (i): the marker check reads every spec's `it`/`describe` titles, line comments stripped (`doc-truth.test.ts:367`); rule 7's marker is `CLIENT_TAKES_PROPS` (`:231`); a marker naming no guard is red (`:352`). N-85 (ii): `describe` titles resolve as tokens (`:139`). Parity: `[P4-X1]`…`[P4-X3]` bound (`criteria-parity.test.ts:217`) |
| **FU-42 · FU-45 · FU-46 · FU-47 · N-79** | parity can't say `[~]` · artifact cap unmeasured · four row shapes · four private strippers · N-79's blind spot | **IN U1** (doc-guard instrumentation). N-14's class continues here. **CLOSED by U1 (2026-09-25):** FU-42: `[~]` is parsed, not ticked, and must be explained (`criteria-parity.test.ts:81`, `:87`, `:238`). FU-45: ARTIFACT_CAP (`:282-331`; 14 files grandfathered, shrink-only). FU-46, **closed in part**: REGISTER_ROW_SHAPE from Phase 4 on (`doc-truth.test.ts:489-563`); the residue is N-89. FU-47 and N-79: the four private strippers are replaced by `src/architecture/__testing__/strip.ts:122` (self-tests `five-xx-is-logged.test.ts:336`). The remainder is registered as N-88 |
...
| **FU-63 · FU-58 · FU-30** | `evidenceProfile` required · stale `paper.ts` header · 4 dead `safetyCopy` helpers | **IN U2.** `labSupported`/`labCaution` are deleted **only if D-3 excludes item 1**: U14a's compliant copy reuses `labSupported`'s template (P-06, P-10). **U2 (2026-09-25):** FU-58 **CLOSED**: `src/types/paper.ts:3-14` states what the type holds today (`doi`/`pmid` only behind a fixture entry, `content/verification/provenance.mjs:3-7,12-26`). FU-30 **closed in part**: `medicationCaution` and `productReasonValue` deleted from `src/lib/safety/index.ts`, zero non-test callers; `labSupported`/`labCaution` held (`:289`, `:298`) and still swept (`safety.test.ts:37-38`). FU-63 **closed in part**: `Effect.evidenceProfile` is required (`src/types/effect.ts:27`); the no-profile branches and their tests it also names are outside U2's *May touch*, and are **N-90** |
| **N-12** | `getRemainingBudget` has no non-test caller | **IN U2**, landed after D-3 is answered. The row's own condition, *"If no UI claims it by phase close, delete it then"*, fell due at Phase 2's close and was not acted on. No Phase 4 candidate renders budget state. **CLOSED by U2 (2026-09-25):** `getRemainingBudget` and its only helper `today()` deleted (`src/lib/advisor/repo.ts:192-196` records it), with its five tests and the stub only they used; `git grep` outside `*.test.ts`, `docs/` and `supabase/migrations/` prints comments only |
...
| **id-manifest move** *(unnumbered; ruling 2026-09-25)* | an id-adding correction hand-edits `src/data/id-manifest.json` | **D-13**, then **U3** if admitted. `project-status.md` registers it as a *candidate* (P-03). **[2026-09-25, U3] CLOSED:** the ledger is `content/id-manifest.json`, and `ID_CORRECTION_DIFF` binds the diff (`[P4-X8]`). **The `[P3-X5]` caveat remains for id removals and renames, and is not closed** (owner, 2026-09-25): U3 proved the id-adding case only |
| **N-47** | two manifest fields, `Namespace.dereferenced` and `manifest.version`, declared and asserted by nothing | **IN U3 if D-13 admits it.** Trigger: *"whichever unit next opens `id-stability.test.ts`"*. Otherwise OUT, trigger intact. **[2026-09-25, U3] CLOSED by option (a), owner's choice:** `id-stability.test.ts` pins `version` to 2 and requires a boolean `dereferenced` in every namespace. Neither field has a policy effect, and the tombstone rule binds `false` namespaces alike. Option (b) was not taken. Proposal and evidence: the U3 record §5 |
...
| **U1** Doc-guard instrumentation · **RUNNER** · **DONE 2026-09-25** | DOC_TRUTH derives tokens and markers from `describe` titles. One declared register row shape. A `[~]`-capable parity row (**D-11 (a)**: the roadmap now carries `[P4-X1]`…`[P4-X3]`). A length assertion on `docs/01-plan/features/*.plan.md`. One shared stripper, string-aware rather than anchored (measured; queue Q-2) | det | L | `src/architecture/**` (new and extended specs; no assertion removed) | N-85, FU-42, FU-45, FU-46, FU-47, N-79; parity per D-11 | N-85's D1/D2 replayed: a rule-7 row flipped to *Not enforced* goes red, a backticked `CLIENT_TAKES_PROPS` goes green. A planted over-cap artifact. A planted `[~]` row. A `//` inside a URL literal. **Evidence:** red and green outputs in `docs/01-plan/features/p4-u1-doc-guards.plan.md` §4 | no |
| **U2** Type and dead-code hygiene · **SUPERVISED** · **DONE 2026-09-25** | **`labSupported`/`labCaution` held** (D-3 admits item 1). `Effect.evidenceProfile` required; `paper.ts` header corrected; dead `safetyCopy` helpers and `getRemainingBudget` deleted (§3 conditions) | det | S | `src/types/{effect,paper}.ts`, `src/lib/safety/**`, `src/lib/advisor/repo.ts`, `src/lib/advisor/repo.test.ts` (its `getRemainingBudget` cases), callers `tsc` names | FU-63 (in part; remainder N-90), FU-58, FU-30 (in part: `labSupported`/`labCaution` held), N-12 | `tsc` fails on a profile-less seed effect. **Deletions: zero *non-test* callers** (`git grep` excluding `*.test.ts`). The test caller at `safety.test.ts:36` (`medicationCaution`; `:38`/`:39` stay, as their helpers are held) and `getRemainingBudget`'s cases in `repo.test.ts` go with them, which **narrows the banned-language sweep** by those entries. The sweep edit is named in the landing. **Evidence (U2):** a seed effect stripped of its profile and regenerated fails `tsc` at `src/data/seed-effects.ts:146` (`creatine-strength`); restored by file copy, `cmp` equal, `content:generate --check` 0 stale. `tsc` named five test callers, all fixtures. Sweep 12 → 11 entries (only `medicationCaution`; `productReasonValue` was never swept). Record: `docs/01-plan/features/p4-u2-type-hygiene.plan.md` | **yes** (sweep narrowing, safety copy; owner-approved text) |
| **U3** *(D-13 (a))* Id manifest out of `src/` · **SUPERVISED** · **DONE 2026-09-25** | Move the manifest into `content/` (D-13 (a)). `id-stability` reads it by path. Rule 16's append-only policy survives unweakened. Settle N-47 | det | M | `content/**`, `src/data/id-manifest.json` (delete), `src/data/id-stability.test.ts`, the 9 GENERATED `src/data/seed-*.ts`, one new spec binding the diff | ruling candidate; `[P3-X5]` caveat (**id-adding case only**; it remains for removals and renames, owner 2026-09-25); N-47 (option (a)) | **(i)** A planted id removal is red at the new location. **(ii) No hand edit of `src/`, stated precisely (P-03):** a recorded id-adding correction whose diff touches `content/**` plus only `src/` files whose line 1 reads `// GENERATED from content/seed/…` (today exactly the 9 `src/data/seed-*.ts`), with CONTENT_FIDELITY (`canonical-layout.test.ts:110`) green, which proves each such file is byte-for-byte generator output. A test binds the name-only diff to that set: red on a planted diff touching the manifest or any non-GENERATED `src/` path. **Named consequences:** `docs/roadmap.md` names the old path at `:45`, `:449`, `:561`, `:577` (a roadmap edit, D-11). **Evidence (U3):** `content/id-manifest.json` is a pure rename, 100% similarity, `cmp` against `HEAD:src/data/id-manifest.json` silent. P-b measured: the GENERATED set is exactly the 9 `src/data/seed-*.ts` (the 10th `seed-*` file is `seed-integrity.test.ts`, a hand-written test). The generator never reads the manifest, so no seed file changed. (i) A registered id dropped from the ledger is red, naming `magnesium`, and a ledger id with no live record is red, naming the tombstone rule at the new path; both restored by file copy, `cmp` equal. (ii) `ID_CORRECTION_DIFF` (R-1, inside `canonical-layout.test.ts`; `SPEC_COUNT` stays 30): four mutations red, and an on-disk dry-run correction's `git diff --name-only` is `content/id-manifest.json`, `content/seed/seed-papers.json`, `src/data/seed-papers.ts`, with no hand-edited path. Record: `docs/01-plan/features/p4-u3-manifest-move.plan.md` | **yes** (rank-1 ledger, append-only policy) |
...
| **U19** *(plan amendment, owner 2026-09-25, recorded at U2)* Scanner-based comment stripping · **RUNNER** | Replace the heuristic in `src/architecture/__testing__/strip.ts` with TypeScript's scanner (`typescript` is already a dependency; U1's review #2 used its parser as ground truth). **Order:** after U3, any time | det | S | `src/architecture/**` | N-88, FU-73 | The five files N-88 names, hidden today, are seen by the model-ID scan; a planted `/*` in a glob string no longer eats code | no |
| **U20** *(plan amendment, owner 2026-09-25, recorded at U3)* FU-63 remainder · **SUPERVISED** | Retire N-90's code part: the no-profile branches that `Effect.evidenceProfile` being required made type-unreachable, their three tests, and the two source comments the tree now contradicts. N-90's accepted parts (the `0008` migration comment, the dated QA record) stay as recorded. **Order:** any time after U3 | det | S | `src/components/library/SupplementDetail.tsx` (`:180`), `src/lib/evidence/index.ts` (`:28`, `:35`), their three tests (`SupplementDetail.test.tsx`, `evidence.test.ts`; N-90 names the lines), `src/types/evidence-grading.ts:33-36`, `tests/e2e/evidence-grading-actions.spec.ts:26` | N-90 (code part); FU-63 (remainder) | *Not stated in the amendment.* It deletes three `it(` blocks, which is stop class 2, so its brief sets the proof | *Not stated in the amendment* |
...
| **N-88** *(U1, 2026-09-25)* | FU-47's inventory was incomplete, found while replacing it. (1) `e2e-live-tagging.test.ts:162` blanks comments with the unanchored `//[^\n]*` before it blanks strings, so a `//` in a URL literal blanks the rest of its line: N-79's class, never listed. (2) The block-comment regex the shared helper keeps is not lexer-aware, and the exposure is **live**: a `/*` inside a line comment (e.g. `` `/v1/*` ``) opens a "comment" that runs to the next `*/`, hiding real code from the model-ID scan in 5 tracked files (`src/lib/advisor/model-adapter.ts:18`, `src/services/advisor-actions.ts:25`, `src/lib/db/advisor-action-repo.ts:2`, `src/lib/advisor/repo.ts:2`, `src/lib/openai/config.ts:53`; found by U1's reviewer, sites re-read). The same regex is why DOC_TRUTH's title and token universes are stripped of line comments only, so text inside a block or JSDoc comment still feeds them (U1's second reviewer). (3) Eight specs keep private copies of the anchored pattern `/^\s*\/\/.*$/gm`, whose `\s*` spans newlines and so shifts line positions. Predicate: files whose line count changes under `.replace(/^\s*\/\/.*$/gm, "")`, over `git ls-files 'src/**/*.ts' 'src/**/*.tsx' 'scripts/**/*.ts' 'scripts/**/*.mjs'` minus `src/architecture/` → 162 of 337 at U1 | **OPEN.** Not fixed in U1: each is a behavioural change to a different guard, and U1's scope was FU-47's four. Owner: the next unit that touches a guard that reads source text |
| **N-89** *(U1, 2026-09-25)* | FU-46's residue: REGISTER_ROW_SHAPE checks rows inside the declared section only. An N-/FU-/OP- id issued elsewhere in a Phase 4+ plan (as N-86 was, in §3) is not detected, and contiguity is checked within the section only (found by U1's reviewer) | **OPEN.** Needs a definition of "issued" that prose mentions do not satisfy. Owner: the next unit that touches `doc-truth.test.ts` |
| **N-90** *(U2, 2026-09-25)* | FU-63's register text names three changes; U2's row and *May touch* admit one. Making `Effect.evidenceProfile` required (`src/types/effect.ts:27`) leaves the no-profile branches as type-unreachable runtime guards: `SupplementDetail.tsx:180` (`e.evidenceProfile &&`) and `src/lib/evidence/index.ts:28,35`. Their tests now build a profile-less effect past the type with a cast (`SupplementDetail.test.tsx:33-49`, `evidence.test.ts:77-96`). Two comments the tree now contradicts are also outside *May touch*: `src/types/evidence-grading.ts:33-36` (*"Optional on Effect in the type only … Making it required is FU-63"*) and `tests/e2e/evidence-grading-actions.spec.ts:26`. With `getRemainingBudget` gone, two present-tense rationales are now false: `supabase/migrations/0008_usage_ledger_policy.sql:37` (*"`getRemainingBudget` reads it"*, as the reason own-row SELECT is allowed; the SELECT is still needed by `listUsageRows`, `src/lib/advisor/repo.ts:291`) and `docs/05-qa/2026-08-12-deployed-schema-record.md:426` (found by U2's reviewer) | **OPEN.** Retiring the branches deletes 3 `it(` blocks (stop class 2) and touches files outside U2's *May touch*: queued for the owner at U2's stop. **Owner ruling (2026-09-25):** *stays open; May touch is not widened.* The migration comment at `0008_usage_ledger_policy.sql:37` is **accepted as-is** (applied migrations are immutable); the QA record at `docs/05-qa/2026-08-12-deployed-schema-record.md:426` **stays as a dated record**. The two unreachable code paths, their three tests and the two source comments get an **owning unit at U3's closeout**. FU-63 stays open, qualified as *closed in part*. **[2026-09-25, U3 closeout] Owning unit: U20** (owner ruling R-4) |
| **N-91** *(U3, 2026-09-25)* | N-47's register row (`phase-2-operational-dependability.plan.md:606`) says `dereferenced` is `false` in *"4 of 9"* namespaces. Measured at every commit of the manifest (`git show <c>:src/data/id-manifest.json`): **1 of 8** at `77b3c36`, **2 of 9** at `ea5b270` (the state when N-47 was written, 2026-08-21), **2 of 10** at `08253e5` and at `d77b36a`. It was never 4. The two are `interactionRules` and `biomarkerRelevanceRules` | **Corrected here, not at `:606`:** a closed phase's register is not rewritten (§7 of `CLAUDE.md`), and that file is outside U3's *May touch*. U3's N-47 proposal uses the measured figure |
| **FU-73** *(U2, 2026-09-25; owner answer to Q-2)* | `src/architecture/__testing__/strip.ts` is a string-aware heuristic, not a lexer (Q-2's known limit), and N-88's block-comment blind spot hides code in 5 tracked files | **OPEN → U19.** Replace the heuristic with TypeScript's scanner |
...
- [x] **[P4-X8]** *(plan-only; D-13 (a))* A content correction that adds an id can be reviewed and shipped without hand-editing `src/`, proven as U3 (ii) states. This closes the `[P3-X5]` caveat. — *U3, 2026-09-25: `ID_CORRECTION_DIFF` in `canonical-layout.test.ts`, with its red and dry-run evidence in the U3 record. Tick approved by the owner. **The `[P3-X5]` caveat remains for id removals and renames, and is not closed** (owner, 2026-09-25). A removal or rename still needs a tombstone and a data migration (rule 16), and U3 did not test that path.*
...
### Owner answers, clarifications and amendment — 2026-09-25 (recorded verbatim at U2)
- **Q-1** (queue): *"(a) U14a revises context-adjusted-evidence.plan.md within 399 lines."*
- **Q-2** (queue): *"(a) keep the string-aware helper. Register FU-<next free>: replace the heuristic in src/architecture/__testing__/strip.ts with TypeScript's scanner (typescript is already a dependency; review #2 used its parser as ground truth); it also closes N-88's block-comment blind spot."* The next free id was **FU-73** (highest issued FU-72, `git grep -ohE 'FU-[0-9]+' -- docs CLAUDE.md`).
- **Q-3** (queue): *"(a) re-measure CLAUDE.md §5's baseline at the Phase 4 closeout only."*
- **Plan amendment:** *"add U19 "Scanner-based comment stripping" · RUNNER · det · S · May touch src/architecture/** · closes N-88 and the new FU · red proof: the five files N-88 names, hidden today, are seen by the model-ID scan; a planted /* in a glob string no longer eats code. Its order: after U3, any time."* Recorded as §4's U19 row.
- **Under D-1 — U1's owner clarifications C-1…C-4** (from U1's brief and artifact, dated 2026-09-25):
  - **C-1** *"U1 adds NO new src/architecture/*.test.ts (SPEC_COUNT pin 30 and its four bound sites are out of scope). Extend existing specs. The shared stripper is a non-test module under src/architecture/ (e.g. src/architecture/_shared/strip.ts)."*
  - **C-2** *"Artifact cap: 200 lines for artifacts created from this landing on. Existing files over 200 at HEAD are pinned by name at their HEAD length on a SHRINK-ONLY list; they may not grow. Do not shorten history."*
  - **C-3** *"Stop-class item 2, "a deleted it(/test(/describe(": a title renamed in place is not a removal. The per-file count of it(/test(/describe( must not decrease; report before/after counts per file."*
  - **C-4** *"The one declared register row shape (FU-46) is enforced on the Phase 4 plan onward. Earlier plans are read tolerantly and never rewritten."*

### Owner rulings — 2026-09-25 (recorded verbatim at U3)
- **R-1** *"The diff-binding test is a new describe inside src/architecture/canonical-layout.test.ts, beside CONTENT_FIDELITY. No new spec file; SPEC_COUNT stays 30."*
- **R-2** *"Old-path mentions in docs/roadmap.md (:45, :449, :561, :577) and the [P3-X5] caveat at docs/project-status.md:501 get a dated note beside the text ("[2026-09-25, Phase 4 U3] moved to content/…"); no line is rewritten or deleted. These edits STOP for the owner (landing)."*
- **R-3** *"N-47: the unit does not decide semantics. It derives what `dereferenced` and `version` were meant to mean from the manifest's own prose and `git log -p` of the file, then proposes: (a) assert both (version pinned; dereferenced present and boolean per namespace) with no policy effect, or (b) make `dereferenced: false` relax the tombstone rule for that namespace, listing every false namespace. STOP with the proposal; no change to ledger semantics without approval."*
- **R-4** *"Plan amendment: add U20 "FU-63 remainder" · SUPERVISED · det · S · closes N-90's code part (SupplementDetail.tsx:180, src/lib/evidence/index.ts:28,35, their three tests, the two source comments) · May touch those files + src/types/evidence-grading.ts:33-36 + tests/e2e/evidence-grading-actions.spec.ts:26 · order: any time after U3. N-90's accepted parts stay as recorded."* Recorded as §4's U20 row.
- **U3 landing approval:** *"Approved exactly as shown: (1) the manifest rename, (2) the five R-2 dated notes with their id-adding-only wording. N-47: option (a), the patch exactly as shown. The [P4-X8] tick is approved. Record in the plan that the [P3-X5] caveat remains for id removals and renames, not closed. Stage, re-run AC-5 and AC-7 on the staged tree, and land under the standing approval."*
