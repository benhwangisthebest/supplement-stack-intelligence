...
**STATUS: NOT RUN. Awaiting the owner's sitting.**
...
| | |
|---|---|
| **Date** | *(fill in — the date the sitting was actually run)* |
| **Run by** | repository owner |
| **Instance** | deployed Supabase — server **PostgreSQL 17.6**, session pooler on 5432 *(confirm; carried from the 2026-08-12 sitting)* |
...
|---|---|---|
| 1.1 | The statement completes with no error | |
...
| Check | Expected | Actual |
|---|---|---|
| 2.1 | `proname` = `delete_all_user_data`, **one row only** | |
| 2.2 | **`args` is EMPTY** | |
| 2.3 | `returns` = `jsonb` | |
| 2.4 | `prosecdef` = `t` | |
...
| Check | Expected | Actual |
|---|---|---|
| 4.1 | **ERROR, SQLSTATE `28000`**, message `delete_all_user_data requires an authenticated caller` | |
| 4.2 | The error names the `RAISE` line of `public.delete_all_user_data()` in its `CONTEXT` | |
...
|---|---|
| 1 — migration applies | |
| 2 — catalog shape (no parameters) | |
| 3 — `search_path` | |
| 4 — null-claim refusal | |

**OP-7 verdict:** *(PASS / FAIL — fill in)*

**Sitting notes:** *(anything observed that the checks above did not anticipate — record it even if it
looked benign at the time; the 2026-08-12 sitting's most useful line was an incidental one about the SQL
editor's list view truncating at 12 rows)*
