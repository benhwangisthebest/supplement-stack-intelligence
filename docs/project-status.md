...
- **[2026-09-22, Phase 2 closeout (d2)]** **1446 unit tests across 114 files**, lint **369 of 369**, **27** architecture specs — re-measured
  ...
  U13–U28 · **[2026-09-22, Phase 2 closeout] BOUND BY `SPEC_COUNT` at `29` — 24 → 26 when the two binding guards landed (they are themselves architecture specs), then → 27 at the (d1) remediation, which added `five-xx-is-logged.test.ts`, then → 28 at Phase 3 U1 landing (b), which added `canonical-layout.test.ts`, then → 29 at Phase 3 U9 landing (a), which added `client-props.test.ts`. Each step moved all four sites in the same commit, because the guard reddens otherwise — which is the first time this count has constrained a landing rather than recording one. Derived from `git ls-files` over `src/architecture/*.test.ts` and asserted against this site: a unit that adds a spec now reddens the build instead of appending a further value here. This is the last hand-written value; do not restack**]**. **[2026-09-22, FU-39: the per-spec test-count breakdown that stood here is DROPPED, not corrected — three of its seven figures were stale (by 22, 8 and 2) and a per-file count restacks on every unit that adds a test. The total above is bound by `SPEC_COUNT`; a breakdown is not worth binding and was not worth carrying unbound. FU-32's remedy, applied a second time.]** Most of them derive
