...
const RELATION_STYLES: Record<BiomarkerRelation, string> = {
  support: "bg-success/10 text-success border-success/30",
      ...
      {rows.length === 0 ? (
        ...
        <ul className="mt-3 space-y-3">
          {rows.map(({ biomarker, rule }) => (
            <li key={rule.id} className="rounded-lg border border-hairline p-4">
              ...
              <p className="mt-1 text-sm text-body">{rule.rationale}.</p>
            </li>
          ))}
        </ul>
      )}

      <p className="mt-3 text-xs text-muted-soft">{DISCLAIMERS.labs}</p>
    </section>
  );
}
